...
class Horror (val _id: String, val _title: String, val _author: String, val _language: String, val _location: String, val _publish: String, val _entry: String, val _status: String) extends Fiction{
    ...
    def saveBook(): Try[Int] = {
        ...
        if (!(isExist)) {
            Try(DB autoCommit {implicit session =>
                sql"""insert into Horror(idNum, bookTitle, author, language, location, publishedDate, 
...
object Horror extends Database{
    def apply(
        _idS: String, 
        _titleS: String,
        _authorS: String,
        ...
        _returnDateS: String,
        _statusS: String
    ): Horror = new Horror(_idS, _titleS, _authorS, _languageS, _locationS, _publishS, _entryS, _statusS){
    ...
    def getAllBook: List[Horror] = {
        DB readOnly {implicit session =>
            sql"select * from Horror".map(rs => Horror(rs.string("idNum"), rs.string("bookTitle"), 
